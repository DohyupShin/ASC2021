...
\end{align*}
    ...
    \noindent
    \begin{solution}
    Let $p, p_0 \in \mathbb{P} = \{p : 0 < p < 1\}$. The pmfs of bi(12, $p$) and bi(12, $p_0$) are $f_p(x) = \binom{12}{x} p^x(1-p)^{12-x}$ and $f_{p_0}(x) = \binom{12}{x} p_0^x(1-p_0)^{12-x}$. Suppose that $p_0$ is given. Then,    
    \begin{align*}
        \frac{f_p(x)}{f_{p_0}(x)} & = (\frac{p}{p_0})^x (\frac{1-p}{1-p_0})^{12-x} \Leftrightarrow \\
        f_p(x) &= (\frac{p}{p_0})^x(\frac{1-p}{1-p_0})^{12-x}f_{p_0}(x) 
        \Leftrightarrow \\    
        f_p(x) &= exp\paren{xlog(\frac{p}{p_0}) + (12-x)log(\frac{1-p}{1-p_0})}f_{p_0}(x) \Leftrightarrow \\
        f_p(x) &= exp\paren{xlog(\frac{p/(1-p)}{p_0/(1-p_0)}) + 12log(\frac{1-p}{1-p_0})}f_{p_0}(x)
    \end{align*}
    We can reparametrize the parameter p to $\alpha = \frac{p}{1-p}$.
    Then, $ f_p(x) = exp\paren{xlog(\frac{p/(1-p)}{p_0/(1-p_0)}) + 12log(\frac{1-p}{1-p_0})}f_{p_0}(x) =  exp\paren{xlog(\frac{\alpha}{\alpha_0}) - 12log(\frac{1+\alpha}{1+\alpha_0})}f_{p_0}(x)$
